...
class MarketAnalyzer {
    ...
    calculateRSI(closes) {
        if (closes.length < 14)
            return 50;
        const rsiValues = technicalindicators_1.RSI.calculate({ values: closes, period: 14 });
    ...
    calculateBollingerBands(closes) {
        ...
        const bb = technicalindicators_1.BollingerBands.calculate({
            values: closes,
            period: 20,
            stdDev: 2
        });
    ...
    calculatePriceChange(closes, periods) {
    ...
    calculateTrendStrength(closes) {
        if (closes.length < 20)
            return 0;
        const sma20 = technicalindicators_1.SMA.calculate({ values: closes, period: 20 });
        const ema12 = technicalindicators_1.EMA.calculate({ values: closes, period: 12 });
    ...
    calculateSupportLevel(lows) {
        if (lows.length < 20)
            return 0;
        const recentLows = lows.slice(-20);
        return Math.min(...recentLows);
    }
    calculateResistanceLevel(highs) {
        if (highs.length < 20)
            return 0;
    ...
    calculateOrderBookImbalance(marketData) {
...
//# sourceMappingURL=marketAnalyzer.js.map